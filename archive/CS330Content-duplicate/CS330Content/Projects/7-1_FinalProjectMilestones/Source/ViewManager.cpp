	...
	ShaderManager *pShaderManager)
{
	...
	// default camera view parameters - EXACT COPY FROM WORKING 3-2 ASSIGNMENT
...
GLFWwindow* ViewManager::CreateDisplayWindow(const char* windowTitle)
{
	...
	// tell GLFW to capture all mouse events and hide cursor for FPS-style camera control
...
void ViewManager::ProcessKeyboardEvents()
{
	...
	// process projection toggle between perspective and orthographic
	// P key for perspective, O key for orthographic
...
void ViewManager::PrepareSceneView()
{
	...
	// toggle between perspective and orthographic projection based on key input
	if (bOrthographicProjection == false)
	{
		// perspective projection (3D view with depth)
		projection = glm::perspective(glm::radians(g_pCamera->Zoom), (GLfloat)WINDOW_WIDTH / (GLfloat)WINDOW_HEIGHT, 0.1f, 100.0f);
	}
	else
	{
		// orthographic projection (2D view, no depth perspective)
		// camera looks directly at objects, bottom plane not visible
		float scale = 10.0f;
